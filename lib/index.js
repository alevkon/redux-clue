...
const DEFAULT_OPTIONS = {
  ...
  clueToRequestUrl: (clue, apiOptions) => {
    ...
    switch (clue.query) {
      case queries.CREATE: {
        const searchHash = getSearchHashByClue(clue, { pick: ["populate", "select", "where"] })
        const search = searchToString(searchHash);
        return `${apiOptions.host}/${apiOptions.apiPrefix}/${apiIdentity}${search}`;
      }
      case queries.UPDATE:
      ...
      case queries.DELETE: {
        ...
        return `${apiOptions.host}/${apiOptions.apiPrefix}/${apiIdentity}/${clue.id}${searchToString(clue.search)}${search}`;
      }
...
export class Clue {
  ...
  toActionType(storeKey, event = events.REQUEST, requestId) {
    switch (event) {
      case events.REQUEST:
        return `${storeKey}:${event}:${this.identity}:${this.query}:${this.serialize()}`;
      case events.SUCCESS:
      case events.ERROR:
        return `${storeKey}:${event}:${this.identity}:${this.query}`;
    }
  }
  ...
  static pseudoClueFromActionType(storeKey, actionType) {
    const parts = actionType.split(':');
    if (parts.length < 4) { return null; }
    if (parts[0] !== storeKey) { return null; }
...
export default (optionsRaw) => {
  ...
  const api = {
    ...
    utils: {
      urlByClue(clue) {
        return apiOptions.clueToRequestUrl(clue, apiOptions)
      }
    },
    selectors: {
      identitySelector,
      ...
      byClue: (propsToClue, optionsArg) => createSelector(
        [
          ...
          (state, props) => {
            const clueData = typeof propsToClue === "function"
              ? propsToClue(props)
              : propsToClue;;
    ...
    actions: {
      update: (identity, id, patch, options) => {
        ...
        return {
          type: clue.toActionType(apiOptions.storeKey),
          payload: {
            options,
      ...
      success: (requestId, clue, responseMeta, data, options) => ({
        type: clue.toActionType(apiOptions.storeKey, events.SUCCESS, requestId),
        payload: {
          requestId,
          clue,
          responseMeta,
      ...
      error: (requestId, clue, responseMeta, data, options) => ({
        type: clue.toActionType(apiOptions.storeKey, events.ERROR, requestId),
        payload: {
          requestId,
          clue,
          responseMeta,
    ...
    reducer: (state = defaultState, action) => {
      ...
      const pseudoClue = Clue.pseudoClueFromActionType(apiOptions.storeKey, type);
            ...
            _.merge(
              ...
              existingItemJSON,
              {
                ...
                meta: new MetaRecord({
                  ...
                  requestAt: existingItemJSON && existingItemJSON.meta.requestAt,
  ...
  const workerSaga = function* (action) {
    const pseudoClue = Clue.pseudoClueFromActionType(apiOptions.storeKey, action.type);
    ...
    switch (pseudoClue.query) {
      ...
      case queries.DELETE: {
        ...
        let response;
        try {
          response = yield call(apiOptions.request || axios.request, Object.assign({}, apiOptions.axios, {
            url,
            method,
            headers,
            data
